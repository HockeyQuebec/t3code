...
// The harness driver resolves workflows through the catalog, so building the
// built-in driver set needs it in context.
it.layer(
  Layer.mergeAll(
    NodeServices.layer,
    ServerSettingsModule.layerTest(),
    TestHttpClientLive,
    HarnessCatalog.layer.pipe(Layer.provide(NodeServices.layer)),
  ),
)("ProviderRegistry", (it) => {
  describe("checkCodexProviderStatus", () => {
    ...
    it.effect.each([
      ...
      Effect.gen(function* () {
        ...
        assert.include(status.message, "Settings → Providers → Codex → Binary path on the server");
        assert.strictEqual(
